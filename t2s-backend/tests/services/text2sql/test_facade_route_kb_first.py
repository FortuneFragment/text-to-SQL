...
class DummySchemaService:
    ...
    def list_table_options_by_names(self, db, table_names):
        ...
        return [
            {"table_name": "t_order", "table_comment": "订单"},
            {"table_name": "t_customer", "table_comment": "客户"},
            {"table_name": "t_product", "table_comment": "商品"},
        ]
...
def test_route_tables_prefers_kb_recall_then_schema_rerank(monkeypatch):
    ...
    monkeypatch.setattr(facade, "_llm_select_best_table", lambda *args, **kwargs: "t_order")

    route = facade._route_tables(
        db=object(),
        question="查询近30天订单和客户信息",
        selected_tables=[],
    )

    assert vector_call_index["value"] == 2
    assert schema.list_table_names_calls == 1
    assert schema.list_table_options_by_names_calls == [["t_order", "t_customer"]]
