...
两个能力，对下编排各子服务，串成一条完整流水线：
    ...
    2. SQL 生成（generator）     ：结合候选表 schema、枚举提示、few-shot 历史样例、
                                   多轮对话上下文，调用大模型生成 SQL。
    3. 校验（validator）         ：白名单表/字段、JOIN 关系白名单、最大表数等安全与合规校验。
    4. 自动修复（repair）        ：校验不通过时把错误信息回灌给大模型迭代修复，最多 N 轮。
    5. 执行（executor）          ：在目标业务库执行 SQL，执行前自动补 TOP、剥离别名、展开 SELECT *。
...
class Text2SQLFacadeService:
    ...
    def __init__(
        ...
        *,
        ...
        外部注入的服务（连接/schema/配置/关系/日志）由 __init__.py 统一构建并复用；
        ...
        self.config_service = config_service  # 连接维度自定义 prompt
        ...
        self.code_hint_service = self._build_code_hint_service()  # 编码列码值提示（生成前编码）
        self.code_decode_service = self._build_code_decode_service()  # 结果码值解码（查询后替换）
    ...
    def _build_code_hint_service(self):
        if not bool(getattr(settings, "TEXT2SQL_CODE_HINT_ENABLED", False)):
            return None
        try:
            from services.text2sql.code_hint_service import Text2SQLCodeHintService

            return Text2SQLCodeHintService(self.config_service.get_connection_key)
        except Exception:  # noqa: BLE001
            _console_logger.exception("code hint service init failed")
            return None

    def _build_code_decode_service(self):
        if not bool(getattr(settings, "TEXT2SQL_CODE_DECODE_ENABLED", False)):
            return None
        try:
            from services.text2sql.code_decode_service import Text2SQLCodeDecodeService

            return Text2SQLCodeDecodeService(self.config_service.get_connection_key)
        except Exception:  # noqa: BLE001
            _console_logger.exception("code decode service init failed")
            return None

    @staticmethod
    def _exclude_coded_columns(
        columns_map: dict[str, set[str]] | None,
        coded_columns: set[tuple[str, str]],
    ) -> dict[str, set[str]]:
        """从可查询字段表剔除已被码值字典覆盖的编码列，供 enum_hint 探测时跳过（避免采样裸码）。"""
        if not columns_map:
            return {}
        ...
        for table, columns in columns_map.items():
            ...
            result[table] = {col for col in columns if (table_upper, str(col).upper()) not in coded_columns}
        return result
    ...
    def _queryable_table_names(
        ...
        if table_names is None:
            return self.schema_service.list_table_names(db)
    ...
    def _queryable_columns_map(
        ...
        """返回 {表名: 可用字段集合}。"""
        return self.schema_service.get_live_table_columns_map(db, table_names)
    ...
    def _run_pipeline(
        ...
        *,
        ...
        # 码值编码：对已绑定字典的编码列，注入 code→中文 映射，让模型用真实 code 过滤。
        coded_columns: set[tuple[str, str]] = set()
        if getattr(settings, "TEXT2SQL_CODE_HINT_ENABLED", False) and self.code_hint_service:
            enhanced_prompt_hint, coded_columns = self.code_hint_service.build_prompt_hint(
                db=db,
                candidate_tables=effective_candidates,
                question=question,
                base_prompt_hint=enhanced_prompt_hint,
            )
        # 枚举提示：把候选字段的可选枚举值喂给模型，减少「状态='已完成'」之类字面量写错。
        # 编码列已由码值字典覆盖，从枚举探测中排除，避免只采样到裸码。
        if settings.TEXT2SQL_ENUM_HINT_ENABLED:
            enhanced_prompt_hint = self.enum_hint_service.build_prompt_hint(
                db=db,
                candidate_tables=effective_candidates,
                queryable_columns_map=self._exclude_coded_columns(candidate_columns_map, coded_columns),
                base_prompt_hint=enhanced_prompt_hint,
            )
        ...
        if execute_sql:
            ...
            field_inference = self.schema_service.build_query_field_comment_bindings(
                db=db,
                columns=columns,
                table_names=effective_candidates,
                queryable_columns_map=result.candidate_columns_map,
            ...
            # 码值解码：把结果里编码列的 code 替换成中文（替换原值；rows 保留原始码）。
            if getattr(settings, "TEXT2SQL_CODE_DECODE_ENABLED", False) and self.code_decode_service:
                decoded_rows = self.code_decode_service.decode_rows(
                    db=db,
                    candidate_tables=effective_candidates,
                    columns=columns,
                    rows=rows,
            ...
            self._emit_progress(
                ...
                "sql_result",
                {
                    ...
                    "rows": list(rows),
                    "decoded_rows": list(decoded_rows),
                    "row_count": len(rows),
                    "repaired": bool(payload.get("repaired")),
            ...
            if progress_callback is None:
                ...
                for chunk in self.summary_service.stream_summarize_result(question, resolved_sql, columns, decoded_rows):
                    if not chunk:
                        continue
                ...
                if not answer:
                    answer = self.summary_service.build_fallback_summary(columns, decoded_rows)
    ...
    def _retry_empty_result_with_enum_probe(
        ...
        *,
        ...
        repaired_sql = self._expand_sparse_detail_projection(repaired_sql, table_columns_map, question)
    ...
    def _extract_literal_filter_columns(
        ...
        if not sql_text:
            return {}
        try:
            tree = parse_one(sql_text, read=INTERNAL_SQLGLOT_DIALECT)
        except Exception:  # noqa: BLE001
            return {}
    ...
    def _evaluate_candidates(
        ...
        *,
        ...
        生成后会统一做后处理：剥离表别名、把 SELECT * 展开为权限内的显式列、补默认 TOP，
        ...
        if not table_columns_map:
            raise ValueError("No available columns found for candidate tables.")
        runtime_config = {
            "selected_tables": candidate_tables,
            "prompt_hint": prompt_hint,
            "few_shot_examples": few_shot_examples,
        ...
        generated_sql = self._expand_sparse_detail_projection(generated_sql, table_columns_map, question)
    ...
    def _route_tables(
        ...
        1. 确定路由范围：若上游指定候选表则以其为准，否则取连接下全部表。
        ...
        if not queryable_tables:
            return {
                ...
                "candidates": [],
                "scores": {},
                "clarify_question": "No queryable tables are available in the current connection.",
            }
    ...
    def _strip_table_aliases(sql: str) -> str:
        ...
        生成的 SQL 统一不使用别名，可让后续的字段校验、SELECT * 展开、字段注释绑定
        ...
        normalized_sql = tree.sql(dialect=INTERNAL_SQLGLOT_DIALECT).strip().rstrip(";")
    ...
    @classmethod
    def _expand_sparse_detail_projection(
        cls,
        sql: str,
        table_columns_map: dict[str, set[str]],
        question: str,
    ) -> str:
        """明细类查询只选了极少字段时，补充真实可查的可读业务字段。

        该规则只处理简单非聚合 SELECT，跳过 DISTINCT、子查询和复杂表达式，避免改写统计语义。
        """
        sql_text = str(sql or "").strip().rstrip(";")
        if not sql_text:
            return "SELECT 1;"
        ...
        expressions = list(tree.expressions or [])
        if not expressions or any(cls._projection_contains_star(expression) for expression in expressions):
            return sql_text + ";"

        table_order = cls._projection_table_order(tree)
        if not table_order:
            return sql_text + ";"

        selected_keys, selected_names = cls._projection_column_keys(tree, table_order)
        if len(selected_keys) >= 4:
            return sql_text + ";"
        if not cls._should_expand_detail_projection(question, expressions, selected_names):
            return sql_text + ";"

        normalized_columns_map = cls._normalized_projection_columns_map(table_columns_map)
        target_count = 6
        expanded_expressions: list[exp.Expression] = list(expressions)
        selected_keys = set(selected_keys)

        for normalized_table, qualifier in table_order:
            if len(selected_keys) >= target_count:
                break
            ranked_columns = cls._rank_display_columns(
                normalized_table,
                normalized_columns_map.get(normalized_table, []),
                selected_keys,
            )
            for column_name in ranked_columns:
                if len(selected_keys) >= target_count:
                    break
                key = (normalized_table, Text2SQLValidatorService.normalize_identifier(column_name))
        ...
        if len(expanded_expressions) == len(expressions):
            return sql_text + ";"
        tree.set("expressions", expanded_expressions)
        normalized_sql = tree.sql(dialect=INTERNAL_SQLGLOT_DIALECT).strip().rstrip(";")
        return normalized_sql + ";"
    ...
    @staticmethod
    def _projection_contains_star(expression: exp.Expression) -> bool:
        if isinstance(expression, exp.Star):
            return True
        if isinstance(expression, exp.Column) and isinstance(expression.this, exp.Star):
            return True
        return expression.find(exp.Star) is not None

    @staticmethod
    def _projection_base_column(expression: exp.Expression) -> exp.Column | None:
    ...
    @classmethod
    def _projection_table_order(cls, tree: exp.Expression) -> list[tuple[str, str]]:
        table_order: list[tuple[str, str]] = []
        seen_tables: set[str] = set()
        for table in tree.find_all(exp.Table):
            table_name = str(table.name or "").strip()
    ...
    def _projection_column_keys(
        ...
        single_table = table_order[0][0] if len(table_order) == 1 else ""
        selected_keys: set[tuple[str, str]] = set()
        selected_names: list[str] = []
        for expression in list(getattr(tree, "expressions", []) or []):
            column = cls._projection_base_column(expression)
            if column is None:
                continue
            normalized_column = Text2SQLValidatorService.normalize_identifier(str(column.name or ""))
            if not normalized_column:
                continue
            selected_names.append(str(column.name or ""))
            raw_table = str(column.table or "").strip()
            normalized_table = normalized_alias_map.get(
                Text2SQLValidatorService.normalize_identifier(raw_table),
                Text2SQLValidatorService.normalize_table_identifier(raw_table) if raw_table else single_table,
            )
            if normalized_table:
                selected_keys.add((normalized_table, normalized_column))
        return selected_keys, selected_names

    @staticmethod
    def _should_expand_detail_projection(
        question: str,
        expressions: list[exp.Expression],
        selected_names: list[str],
    ) -> bool:
        if any(Text2SQLFacadeService._projection_base_column(expression) is None for expression in expressions):
            return False
        question_text = str(question or "").strip().lower()
        if not question_text:
            return False
        narrow_markers = ("只要", "仅返回", "只显示", "只列出", "只看", "有哪些字段", "字段名", "列名")
        if any(marker in question_text for marker in narrow_markers):
            return False
        strong_markers = ("明细", "详情", "详细", "信息", "数据", "记录", "列表", "清单", "名单", "台账", "档案")
        if any(marker in question_text for marker in strong_markers):
            return True
        weak_markers = ("查询", "查看", "显示", "列出", "给我", "查一下")
        if any(marker in question_text for marker in weak_markers):
            return len(selected_names) <= 1 and any(
                Text2SQLFacadeService._looks_like_identifier_column(name) for name in selected_names
            )
        return False
    ...
    def _looks_like_identifier_column(column_name: str) -> bool:
        ...
        return (
            name in {"id", "uuid", "guid", "code", "no", "num", "number"}
            or name.endswith("_id")
            or name.endswith("_code")
            or name.endswith("_no")
            or name.endswith("_num")
            or name.endswith("编号")
            or name.endswith("代码")
        )

    @staticmethod
    def _normalized_projection_columns_map(table_columns_map: dict[str, set[str]]) -> dict[str, list[str]]:
        result: dict[str, list[str]] = {}
        for table_name, columns in (table_columns_map or {}).items():
            normalized_table = Text2SQLValidatorService.normalize_table_identifier(table_name)
            if not normalized_table:
                continue
            result[normalized_table] = sorted(
                [str(column).strip() for column in (columns or set()) if str(column).strip()]
            )
        return result

    @classmethod
    def _rank_display_columns(
        cls,
        ...
        candidates = [
            ...
            if (normalized_table, Text2SQLValidatorService.normalize_identifier(column)) not in selected_keys
            and cls._display_column_priority(column) >= 0
        ]
        return sorted(candidates, key=lambda column: (-cls._display_column_priority(column), column.lower()))

    @staticmethod
    def _display_column_priority(column_name: str) -> int:
        name = str(column_name or "").strip().lower()
        if any(token in name for token in ("password", "passwd", "secret", "token", "salt", "hash")):
            return -10
        if any(token in name for token in ("name", "title", "姓名", "名称", "名字", "标题", "display", "label")):
            return 90
        if any(token in name for token in ("account", "number", "code", "no", "编号", "学号", "工号", "账号", "代码")):
            return 70
        if any(token in name for token in ("class", "grade", "dept", "major", "班级", "年级", "部门", "学院", "专业")):
            return 60
        if any(token in name for token in ("status", "state", "type", "category", "状态", "类型", "类别", "分类")):
            return 50
        if any(token in name for token in ("date", "time", "year", "semester", "created", "updated", "日期", "时间", "学年", "学期")):
            return 40
        if any(token in name for token in ("score", "amount", "money", "count", "分数", "成绩", "金额", "数量")):
            return 30
    ...
    def _strip_top_level_limit(sql_text: str, tree: exp.Expression | None) -> str:
        """去掉顶层 TOP：统计类查询若被截断会导致聚合结果不完整，故先剥离。"""
        if tree is not None and tree.args.get("limit") is not None:
            copied = tree.copy()
            copied.set("limit", None)
            return copied.sql(dialect=INTERNAL_SQLGLOT_DIALECT).strip().rstrip(";")
        return re.sub(
    ...
    def _ensure_limit(cls, sql: str) -> str:
        """明细查询自动补默认 TOP（防止全表拉取拖垮业务库）；统计类查询不截断。"""
        sql_text = str(sql or "").strip().rstrip(";")
        max_rows = int(settings.TEXT2SQL_MAX_ROWS)
        if not sql_text:
            return f"SELECT TOP ({max_rows}) 1;"
        ...
        if tree is not None and tree.args.get("limit") is not None:
            return tree.sql(dialect=INTERNAL_SQLGLOT_DIALECT).strip().rstrip(";") + ";"
        if re.search(r"\btop\s*\(?\d+\)?", sql_text, flags=re.IGNORECASE):
            return sql_text + ";"
        if tree is not None and isinstance(tree, exp.Select):
            copied = tree.copy()
            copied.set("limit", exp.Limit(expression=exp.Literal.number(max_rows)))
            return copied.sql(dialect=INTERNAL_SQLGLOT_DIALECT).strip().rstrip(";") + ";"
        limited_sql = re.sub(
    ...
    def _get_llm_client(self, *, streaming: bool) -> ChatOpenAI | None:
        ...
        """
        runtime_config = model_config_service.get_active_runtime_config(MODEL_KIND_LLM)
        if runtime_config is None:
            return None
        base_url = normalize_llm_base_url(runtime_config.base_url)
        api_key = str(runtime_config.api_key or "").strip()
        model_name = str(runtime_config.model_name or "").strip()
        if not (base_url and model_name):
            return None

        base_key = runtime_config.cache_key
        if streaming:
        ...
        client = ChatOpenAI(
            ...
            request_timeout=int(runtime_config.timeout_seconds),
            streaming=streaming,
        )
        if streaming:
